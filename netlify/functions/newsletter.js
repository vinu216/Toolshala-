...
exports.handler = async (event) => {
  ...
  const source = normalizeText(body.source || 'newsletter');
  const page = normalizeText(body.page || event.headers?.referer || event.headers?.referrer || '');
  ...
  if (!url || !serviceRoleKey) return jsonResponse(500, { error: 'Newsletter storage is not configured on the server.' });
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 409 || data?.code === '23505') return jsonResponse(200, { ok: true, duplicate: true });
      return jsonResponse(502, { error: String(data?.message || data?.error || '').trim() || 'Could not save the subscriber. Please try again.' });
    }

    return jsonResponse(200, { ok: true });
  } catch (error) {
    if (error?.name === 'AbortError') return jsonResponse(504, { error: 'Newsletter request timed out. Please try again.' });
    return jsonResponse(502, { error: 'Could not save the subscriber. Please try again.' });
  } finally {
