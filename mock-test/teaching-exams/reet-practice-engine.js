(() => {
  ...
  const config = window.reetPracticeConfig || {};
  const bank = window.reetPracticeQuestionBank || { sections: [] };
  ...
  function renderQuestion() {
    if (!data.length) {
      ...
      $('question-text').textContent = 'REET practice question data could not be loaded.';
      return;
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    if (!section) {
      $('practice-title').textContent = 'REET Practice Set';
      $('practice-description').textContent = 'Practice section not found. Please return to the REET Practice Sets hub.';
      $('question-meta').textContent = 'Questions unavailable';
      return;
    }
    bindEvents();
