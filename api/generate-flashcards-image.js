...
export default async function handler(req, res) {
  ...
  const apiKey = String(process.env.FLASHCARD_IMAGE_API_KEY || process.env.VISION_API_KEY || process.env.OPENAI_API_KEY || '').trim();
  if (!apiKey) return res.status(500).json({ error: 'Flashcard image generator is not configured. Add FLASHCARD_IMAGE_API_KEY, VISION_API_KEY, or OPENAI_API_KEY on the server.' });
